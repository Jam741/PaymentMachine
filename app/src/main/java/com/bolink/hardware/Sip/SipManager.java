package com.bolink.hardware.Sip;
...
public class SipManager implements LinphoneCoreListener {
    ...
    public static final String SIP_TERMINATE = "Terminating the call";
    public static final String SIP_RINGING = "LinphoneCall.State.OutgoingRinging";
    private boolean IsRinging = false;
    public static final String SIP_TALKING = "LinphoneCall.State.StreamsRunning";
    private boolean IsTalking = false;
    public static final String SIP_INIT = "Init";

    public static final String SIP_INCOMMING = "Callincoming";
    private boolean IsIncomming = false;

    public SipManager(Context c) {
        mContext = c;
        LinphoneCoreFactory.instance().setDebugMode(false, "KEYTOP_SIP");
...
//        EventBus.getDefault().post(event);
    ...
    public void displayStatus(LinphoneCore linphoneCore, String s) {
        Log.i(TAG, "displayStatus  :" + s);
        if (s.contains("is contacting you")) {
            //有来电-自动接通
//            RxBus.get().post(new Messages(Messages.LINEPHONE_MSG, SIP_INCOMMING));
            IsIncomming = true;
//            try {
//                acceptCall();
//                IsTalking = true;
...
//        EventBus.getDefault().post(event);
    ...
    private int currentmil = 0;
    private boolean incomingswitch = false;
    ...
    public void callState(LinphoneCore linphoneCore, LinphoneCall linphoneCall, LinphoneCall.State state, String s) {
        ...
        if (SIP_INCOMMING.equals(linphoneCall.getDirection().toString())) {
            if (IsIncomming) {
                if (incomingswitch)
                    return;
                incomingswitch = true;
...
//            LinphoneCallParams params = linphoneCall.getCurrentParamsCopy();
//            params.setVideoEnabled(true);
//            try {
//                mLinphoneCore.acceptCallUpdate(linphoneCall, params);
//
//                // 开始显示视频
//
//                Message msg = new Message();
//                msg.what = 1;
//                ((MainActivity)mContext).h.sendMessage(msg);
//
//            } catch (LinphoneCoreException e) {
//                // TODO Auto-generated catch block
//                e.printStackTrace();
