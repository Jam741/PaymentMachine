            ...
            that.timeInterval = setInterval(function () {
                if (count > 0) {
                    ...
                    if (that.interval_codepay_back) {
                        ...
                        //var paramss = that.baseUrl + '/' + codeurl + '?token=' + that.token + '&order_id=' + that.order_id + '&plate_number=' + that.plate_number;
                        //console.log(that.baseUrl + '/' + codeurl + '?token=' + that.token + '&order_id=' + that.order_id + '&plate_number=' + that.plate_number)
                        that.$http.get(that.baseUrl + '/' + codeurl + '?token=' + that.token + '&order_id=' + that.order_id + '&plate_number=' + that.plate_number, {timeout: TimeOutMil})
                            .then(function (res) {
                                ...
                                that.loading = false;
								WriteLog('120s的扫码的轮询'+res.data.state)
                                if (res.data.state == 1) {
                                    // 扫码支付成功,返回首页
