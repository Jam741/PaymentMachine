...
//            alert(msg);
        ...
        clickSearch: function () {
            ...
            if (this.carnumber.indexOf("ADVANCE") != -1) {
				...
				Print( '预付费存根', '测试停车场',  '中央电梯缴费机',  '京A12345',  '20元',  '现金支付',  '2017-10-12 15:17:22',  '请在免费时间段内尽快离场，超时将补缴停车费');
			}
            this.oid = '';
            if (this.internet) {
                this.loading = true;
                ...
                this.$http.get(this.baseUrl + '/getprepayqrcode?order_id=' + this.order_id + '&token=' + this.token)
                    .then(function (res) {
                        ...
                        if (res.data.state == 1) {
							...
							//alert('creattttttt  '+this.order_id+' '+this.plate_number+' '+this.order_intime+' '+this.order_price+' '+this.order_price);
							//alert(this.order_id);
							//alert(this.plate_number);
							//alert(new Date(res.data.orders[0].inTime * 1000).toLocaleString()+'');
							//alert(this.order_price);
                    ...
                    that.timeInterval = setInterval(function () {
                        if (that.count > 0) {
                            that.count--;
							   ...
							   this.$http.get(this.baseUrl + '/mobilerecharge?oid=' + this.oid + '&token=' + this.token + '&change=' + this.cashReturn + '&mobile=' +  '&trade_no=' )
                        .then(function (res) {
                    ...
                    this.$http.get(this.baseUrl + '/cashprepay?oid=' + this.oid + '&token=' + this.token + '&total=' + this.order_price + '&change=' + this.cashReturn + '&receive=' + this.getCash)
                        .then(function (res) {
                            ...
                            if (res.data.state == 1) {
                                ...
                                Print( '预付费存根', this.parkName,  this.centralpaymentName,  this.carnumber,  this.order_price,  '现金支付',  this.order_intime,  this.freeOutTime);
                            } else {
                                this.alertMsg(res.data.errmsg)
                            }
                        }).catch(function (e) {
